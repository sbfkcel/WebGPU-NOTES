...
const initPipeline = async (device:GPUDevice, format:GPUTextureFormat)=>{
    // 声明顶点数据
    const vertexXyData = new Float32Array([
        // xy
        0,0.5,
        -0.5,-0.5,
        0.5,-0.5,
    ]);
    const vertexZData = new Float32Array([
        // z
        0,
        0,
        0
    ...
    // 在GPU中创建一个Buffer
    const vertexXyBuffer = device.createBuffer({
        // buffer字节大小，Float32Array一个数字占4个字节，所以是 9*4。也可直接调用 byteLength 获取
        size:vertexXyData.byteLength,
        // 设置 Buffer 用途，这里选择为 VERTEX 即可
        usage:GPUBufferUsage.VERTEX | GPUBufferUsage.COPY_DST
    });
    const vertexZBuffer = device.createBuffer({
        // buffer字节大小，Float32Array一个数字占4个字节，所以是 9*4。也可直接调用 byteLength 获取
        size:vertexZData.byteLength,
        // 设置 Buffer 用途，这里选择为 VERTEX 即可
        usage:GPUBufferUsage.VERTEX | GPUBufferUsage.COPY_DST
    });
    ...
    device.queue.writeBuffer(vertexXyBuffer,0*4,vertexXyData);
    device.queue.writeBuffer(vertexZBuffer,0*4,vertexZData);
    ...
    const vertexObj = {vertexXyData,vertexXyBuffer,vertexCount:3,vertexZData,vertexZBuffer};
...
const draw = (device:GPUDevice, pipeline:GPURenderPipeline, context:GPUCanvasContext, vertexObj:any, colorObj:any)=>{
    ...
    // 向第 0 个通道中设置 vertexBuffer 数据（xy）
    renderPass.setVertexBuffer(0, vertexObj.vertexXyBuffer);

    // 向第 1 个通道中设置 vertexBuffer 数据（z）
    renderPass.setVertexBuffer(1, vertexObj.vertexZBuffer);
...
const fun = async()=>{
    ...
    range.addEventListener('input',(e)=>{
        ...
        // 由于第 0 个通道中的数据只包含了 xy 数据，所以 x、y、z 对应的是 0、2、4
        vertexObj.vertexXyData[0] = 0 + value;
        vertexObj.vertexXyData[2] = -0.5 + value;
        vertexObj.vertexXyData[4] = 0.5 + value;
        // 将新的数据写入到 vertexBuffer 中并重新绘制当前图形
        device.queue.writeBuffer(vertexObj.vertexXyBuffer,0,vertexObj.vertexXyData);
        draw(device, pipeline, context, vertexObj, colorObj);
    });
