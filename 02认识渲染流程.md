...
跟 commandEncoder 类似 renderPass 其实相当于 sub-encoder ，它会返回一个 runderPass 的对象。我们可以将 runderPass 将真正的绘制命令添加到每个通道中去，接下次来开始真正的绘制工作,当所有的绘制工作完成之后调用 renderPass.end() API 来结束这个通道的录制工作，最终相关的命令都会被写入到 encoder 中。

runderPass 必须传入 renderPassDescripotor 的参数，参数用来告诉该通道如何处理绘制的内容。一般情况下每一帧都会替换前一帧的内容，也就是说一般会先清空画布然后再进行新的绘制工作，那么这就需要设置一个背景色。可以通过 colorAttachments 颜色附件来告诉这个 pass 中如何处理相关的颜色信息，其包括一个 view ，也就是最终想要这个通道输出在哪里显示，当然是直接输出在最终的画布上，所以这里通过之前获取到的 context.getCurrentTexture() 再调用 createView 来获取一个可以被 GPU 操作的 view buffer。
...
对应的还要添加一个背景颜色，使用 clearValue 来进行设置，需要传入一个 rgba 的颜色。
